...
#define PLAYER_DEFAULT_VELOCITY FIX16(5.0)
...
static const fix16 zero = FIX16(0.0);
static const fix16 PLAYER_MIN_Y=intToFix16(1*8);
static fix16 PLAYER_MAX_Y;
...
void loadSprites()
{
    ...
    Sprite_init(&ballSpr, &ball, PAL3, 1, FIX16(380.0), FIX16(380.0), FIX16(0.0), FIX16(0.0));
...
void Game_initData(Game *game)
{
    if (!gameInitialized)
    {
         PLAYER_MAX_Y=intToFix16(g_screen_height-8-PADDLE_SIZE);   
        initLevels();
        ...
        game->player1.paddleVelocity=PLAYER_DEFAULT_VELOCITY;
        game->player2.paddleVelocity=PLAYER_DEFAULT_VELOCITY;
...
void clearPaddleVelocity(SpriteObj *sprite) {
    Sprite_setVelocity(sprite,zero,zero);    
}
...
void Game_processInput(Game *game, u16 button,bool up)
{
    ...
    case BUTTON_UP:
        ...
        if (game->state == GAME_PLAYING)
        {
            if (up)
            {
                clearPaddleVelocity(game->player1.sprite);
            }
            else
            {
                Sprite_setVelocityY(game->player1.sprite,fix16Neg(game->player1.paddleVelocity));               
            }            
        }
        break;
    ...
    case BUTTON_DOWN:
        ...
        if (game->state == GAME_PLAYING)
        {
            ...
            {
                Sprite_setVelocityY(game->player1.sprite,game->player1.paddleVelocity);               
            }            
        }
        break;
    }
...
void constraintPaddlePositionY(SpriteObj *sprite)
{
    if (sprite->y<=PLAYER_MIN_Y)
    {
        Sprite_setYPos(sprite,PLAYER_MIN_Y);
        clearPaddleVelocity(sprite);
    }   
    else if (sprite->y>=PLAYER_MAX_Y)
    {
        Sprite_setYPos(sprite,PLAYER_MAX_Y);
        clearPaddleVelocity(sprite);                
    }
}

void updatePlayer1(Game *game)
{
    const SpriteObj *sprite=game->player1.sprite;
    Sprite_updatePosByVelocity(sprite);
    constraintPaddlePositionY(game->player1.sprite);
    Sprite_update(game->player1.sprite);    
}
